...
| **持仓组合分析** | 从持仓截图识别持仓，诊断组合健康度、风险和配置 | "帮我分析一下持仓"（附截图） |
...
发送券商持仓截图，AI 自动识别持仓并综合分析组合健康度：

```
你：帮我看看持仓（附截图 1.jpg 2.jpg）

AI：（识别持仓 → 确认投资风格 → 获取行情和指标 → 输出诊断报告）
    持仓总览、资产配置、盈亏诊断、个股点评、风险提示、优化思路
```

支持 A 股个股、ETF、港股混合持仓。会根据你的投资风格（长线/短线、激进/保守）定制分析视角。
...
| [数据源参考手册](docs/data-sources.md) | 各接口能力、性能、选用建议 |

---

## 数据来源

- **行情数据**：新浪财经、腾讯股票（公开接口，免费，<1 秒响应）
- **财务数据**：[akshare](https://github.com/akfamily/akshare)（开源，免费，批量全市场）

无需申请 API Key。各接口的能力、性能和选用建议见 **[数据源参考手册](docs/data-sources.md)**。
